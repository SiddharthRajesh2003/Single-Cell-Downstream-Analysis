...
seurat_list.names <- names(seurat_list)
```
...
ggsave('plots/scatter_plots_before_qc.png', combined_plot, width = 70, height = 50, dpi = 300, limitsize = FALSE)
```
...
ggsave('plots/scatter_plots_after_qc.png', combined_plot, width = 70, height = 50, dpi = 300, limitsize = FALSE)
```
...
# Finding the variable features in each sample
...
names(seurat_list) <- seurat_list.names
```
...
for (sample_name in names(seurat_list)){
    ...
    sample.genes <- rownames(sample.obj)
    sample.obj <- ScaleData(sample.obj, features = sample.genes)
    
    seurat_list[[sample_name]] <- sample.obj
}

seurat_list <- lapply(X = seurat_list, FUN = function(X){
    sample.genes <- rownames(X)
    X <- ScaleData(X, features = sample.genes)
})
names(seurat_list) <- seurat_list.names
```
...
seurat_list <- lapply(X = names(seurat_list), FUN = function(name){
    X <- seurat_list[[name]]
    cat(paste0("\nSeurat Object Name:\t",name,'\n'))
...
## PCA Plots
```{r}
plot_PCA <- function(seurat_list){
    pca_plots <- list()

    for (sample_name in sample_names) {
        sample.obj <- seurat_list[[sample_name]]
        
        pca.plot <-  DimPlot(sample.obj, reduction = 'pca', pt.size = 0.6) +
            ggtitle(paste0('PCA - ', sample_name))
        
        pca_plots[[sample_name]] <- pca.plot
    }
    return(pca_plots)
}
pca.plots <- plot_PCA(seurat_list)
combined_plot <- wrap_plots(pca.plots)
ggsave('plots/PCA_plots.png', combined_plot, width = 70, height = 50, dpi = 300, limitsize = FALSE)
```
